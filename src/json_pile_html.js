...
obj.set = function (key, val, options) {
    ...
    if (
        (typeof key === "string" && !/^(\w+)\[/.test(key)) ||
        (typeof key === "object" &&
            !_.filter(_.keys(key), (k) => /^(\w+)\[/.test(k)).length)
    ) {
...
obj.parse = function (response, options) {
    if (this.jsonPile !== false) {
        for (var x in response) {
            if (
                typeof response[x] === "object" &&
                (typeof this.jsonPile === "boolean" ||
                    _.indexOf(this.jsonPile, x) !== -1)
            ) {
...
obj.toJSON = function (options) {
    var clone = this.getJSONtoFormat(options),
        json1 = {},
        json2 = {};
    ...
    if (!this.removePileLists) {
        ...
        for (let x in clone) {
            ...
            if (
                !/^upload/.test(x) &&
                (!this.removePileLists || typeof clone[x] !== "object") &&
                (typeof this.removePileLists === "boolean" ||
                    _.indexOf(this.removePileLists, listName) === -1)
            ) {
                //            console.log(x);
                //            console.log(clone[x]);
                json1[x] = clone[x];
    ...
    json1 =
        this.formatOnToJSON && typeof this.formatData === "function"
            ? this.formatData(json1, options, 1)
            : json1;
